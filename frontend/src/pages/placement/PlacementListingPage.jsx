    ...
    <div>
      ...
      {fetching ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {items.map((item) => (
            <div key={item._id}
              onClick={() => navigate(`/placement/${type.replace('_', '-')}s/${item._id}`)}
              className="bg-white rounded-xl shadow-md hover:scale-105 hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden cursor-pointer">
              {/* Banner */}
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <div className="flex gap-1.5 mt-auto pt-3 border-t border-slate-100">
                  <button onClick={(e) => { e.stopPropagation(); navigate(`/placement/${type.replace('_', '-')}s/${item._id}/applicants`); }}
